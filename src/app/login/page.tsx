...
// Callback Google mengarahkan kegagalan ke /login?error=<code>. Pesan diambil dari peta ini, bukan
// dari query string, supaya teks pada halaman tidak pernah berasal dari input penyerang. Kode yang
// tidak dikenal tidak menampilkan apa pun.
const errorMessages: Record<string, string> = {
  SIGNUP_RATE_LIMITED: "Pendaftaran akun baru sedang dibatasi. Coba lagi dalam satu jam.",
  LOGIN_RATE_LIMITED: "Terlalu banyak percobaan masuk. Coba lagi beberapa saat lagi.",
  INVALID_OAUTH_STATE: "Sesi login sudah berakhir atau terpakai. Silakan mulai lagi.",
  GOOGLE_LOGIN_CANCELLED: "Login Google dibatalkan. Silakan coba lagi.",
  GOOGLE_AUTH_FAILED: "Verifikasi Google gagal. Silakan coba lagi.",
  ACCOUNT_NOT_APPROVED: "Akun Google ini belum memiliki akses. Hubungi administrator.",
  GOOGLE_NOT_CONFIGURED: "Login Google belum dikonfigurasi administrator.",
  INVALID_CALLBACK: "Callback Google tidak valid. Silakan mulai lagi.",
  AUTH_UNAVAILABLE: "Layanan autentikasi sedang tidak tersedia. Coba lagi nanti.",
};

export default async function LoginPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const devLogin = isDevRoleLoginEnabled();
  const code = (await searchParams).error;
  const alert = typeof code === "string" ? errorMessages[code] : undefined;
  return (
