...
// Nilai-nilai ini harus cocok dengan userRoleFilters/userSortKeys di src/server/auth/admin-user-list.ts;
// nilai asing ditolak 422 oleh API, jadi dropdown-nya tidak boleh menyimpang.
const roleFilterOptions = [...roleOptions, "tanpa_role"] as const;
const sortOptions = [
  { value: "email", label: "Email (A–Z)" },
  { value: "nama", label: "Nama (A–Z)" },
  { value: "terbaru", label: "Terbaru dibuat" },
  { value: "terlama", label: "Terlama dibuat" },
  { value: "role", label: "Role (admin dulu)" },
] as const;
...
export default function AdminOperations({ kind, actorId }: { kind: Kind; actorId: string }) {
  ...
  // Khusus kind="users". Disimpan tanpa syarat (hook tidak boleh kondisional) tapi hanya dikirim
  // untuk halaman akun: skema .strict() adminPage menolak parameter yang tidak dikenal halaman lain.
  ...
  const endpoint = useCallback((cursor?: string) => {
    ...
    if (kind === "users" && roleFilter) parameters.set("role", roleFilter);
    if (kind === "users" && sort !== "email") parameters.set("sort", sort);
    if (cursor) parameters.set("cursor", cursor);
    return "/api/v1/admin/" + kind + "?" + parameters;
  }, [kind, query, status, roleFilter, sort]);
  useEffect(() => {
