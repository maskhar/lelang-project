...
export async function POST(request:NextRequest){try{requireCsrf(request);const actor=await getAuthenticatedActor({ allowNoRole: true });await consumeRateLimit("access-request:global",100,60);await consumeRateLimit("access-request:"+actor.profileId,5,3600);const value=input.parse(await readAuthJson(request));const database=getDatabase();const [data]=await database.transaction(async tx=>{const existing=await tx.select({id:accessRequests.id}).from(accessRequests).where(and(eq(accessRequests.profileId,actor.profileId),eq(accessRequests.status,"pending"))).limit(1).for("update");if(existing.length)throw new AuthHttpError(409,"REQUEST_PENDING","Masih ada pengajuan yang sedang diproses.");const [request]=await tx.insert(accessRequests).values({profileId:actor.profileId,requestedRole:value.requestedRole,reason:value.reason}).returning();await tx.insert(auditLogs).values({actorId:actor.profileId,action:"access_request.created",entityType:"access_request",entityId:request.id});return [request]});return NextResponse.json({data},{status:201});}catch(error){return apiErrorResponse(error);}}
