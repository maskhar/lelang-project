...
export async function GET(){try{requireRole(await getAuthenticatedActor(),"admin");const data=await getDatabase().select({id:propertyAssignments.id,propertyId:properties.id,title:propertyRevisions.title,agentId:profiles.id,agentName:profiles.name,agentEmail:profiles.email,note:propertyAssignments.note,assignedAt:propertyAssignments.assignedAt}).from(propertyAssignments).innerJoin(properties,eq(properties.id,propertyAssignments.propertyId)).innerJoin(propertyRevisions,eq(propertyRevisions.propertyId,properties.id)).innerJoin(profiles,eq(profiles.id,propertyAssignments.agentId)).where(isNull(propertyAssignments.unassignedAt)).orderBy(desc(propertyAssignments.assignedAt));return NextResponse.json({data},{headers:{"Cache-Control":"no-store"}})}catch(e){return apiErrorResponse(e)}}
