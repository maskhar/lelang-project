...
import { bulkArchiveListings, deleteListings, staffListings } from "@/server/properties/service";
...
export async function DELETE(request: NextRequest) { try { requireCsrf(request); const actor = await getAuthenticatedActor(); const body = await readAuthJson(request); return NextResponse.json({ data: await deleteListings(actor, body) }, { headers: { "Cache-Control": "no-store" } }); } catch (error) { return apiErrorResponse(error); } }
