...
export default async function PropertyDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PropertySiteChrome><main className={styles.page}>
    ...
    <section className={styles.content}><article><section className={styles.descriptionCard} aria-labelledby="description-title"><h2 id="description-title">Deskripsi Aset</h2><p>{property.desc || "Deskripsi belum tersedia."}</p></section><section className={styles.specificationCard} aria-labelledby="specification-title"><h2 id="specification-title">Spesifikasi Aset</h2><dl className={styles.assetSpecs}>{specifications.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section><section className={styles.locationMap} aria-labelledby="location-map-title"><div><span>LOKASI ASET</span><h2 id="location-map-title">Peta lokasi</h2><p>{mapLabel}</p></div><iframe title={`Peta lokasi ${property.title}`} src={`https://www.google.com/maps?q=${encodeURIComponent(mapQuery)}&output=embed`} loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></section><PropertyAmenities /></article><aside className={styles.verification}><span>INFORMASI PENTING</span><h2>Periksa aset sebelum transaksi</h2><p>Pelajari dokumen, status kepemilikan, kondisi fisik, serta ketentuan penawaran sebelum mengambil keputusan.</p><ul><li>Atur jadwal pengecekan lokasi</li><li>Verifikasi dokumen dan legalitas</li><li>Pastikan detail transaksi disepakati para pihak</li></ul></aside></section>
