...
export default async function PropertyDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isSafeInteger(propertyId) || propertyId < 1) notFound();
  ...
  const recommendations = getProperties()
  ...
  return <PropertySiteChrome><main className={styles.page}>
    <div className={styles.breadcrumb}>Katalog properti <span>/</span> {property.type} <span>/</span> #{property.id}</div>
    ...
    <div className={styles.publicationInfo}><span>Dipublikasikan: belum tersedia <small>(Tanggal pencatatan: {dateLabel})</small></span><span>Dikelola oleh: belum tersedia</span></div>
    ...
    <section className={styles.contact} id="hubungi"><div><span>TERTARIK DENGAN PROPERTI INI?</span><h2>{property.mode === "lelang" ? "Ajukan penawaran sebelum waktu berakhir." : "Diskusikan properti ini dengan pemilik."}</h2><p>Prototype lokal. Kontak dan transaksi nyata belum diaktifkan.</p></div><Link className="button light" href="/#properti">Kembali ke katalog</Link></section>
    </div><aside className={styles.sidebar}>
      <div className={styles.summary}><span className={property.mode === "lelang" ? styles.lelang : property.mode === "terjual" ? styles.terjual : styles.langsung}>{statusLabel(property.mode)}</span><p className={styles.location}>⌖ {property.city}</p><h1>{property.title}</h1><p className={styles.type}>{property.type} · ID properti #{property.id}</p><div className={styles.priceBox}><span>{property.mode === "lelang" ? "Penawaran tertinggi" : "Harga penawaran"}</span><strong>{formatRupiah(offer)}</strong><small>{formatExactRupiah(offer)}</small>{property.mode === "lelang" && <em>{property.bidders ?? 0} penawar tercatat</em>}</div><a className="button dark wide" href="#hubungi">{property.mode === "lelang" ? "Ajukan penawaran" : property.mode === "terjual" ? "Lihat properti serupa" : "Hubungi pemilik"}</a><PropertyActions id={property.id} title={property.title} /></div>
    </aside></div>
