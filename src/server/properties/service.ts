...
function revisionValues(input: Listing) {
  return { title: input.title, description: input.description, address: input.address, landAreaM2: input.landAreaM2, buildingAreaM2: input.buildingAreaM2, bedroomCount: input.bedroomCount, amenities: input.amenities, auctionStartsAt: input.auctionStartsAt ? new Date(input.auctionStartsAt) : null, auctionEndsAt: input.auctionEndsAt ? new Date(input.auctionEndsAt) : null };
}
...
export async function publicListing(slug: string) {
  const [row] = await getDatabase().select({ id: properties.id, sku: properties.sku, revisionId: properties.publishedRevisionId, slug: properties.slug, saleMode: properties.saleMode, availabilityStatus: properties.availabilityStatus, type: properties.type, askingPrice: properties.askingPrice, publishedAt: properties.publishedAt, title: propertyRevisions.title, description: propertyRevisions.description, location: propertyRevisions.listingSnapshot, landAreaM2: propertyRevisions.landAreaM2, buildingAreaM2: propertyRevisions.buildingAreaM2, bedroomCount: propertyRevisions.bedroomCount, amenities: propertyRevisions.amenities, auctionStartsAt: propertyRevisions.auctionStartsAt, auctionEndsAt: propertyRevisions.auctionEndsAt }).from(properties).innerJoin(propertyRevisions, eq(properties.publishedRevisionId, propertyRevisions.id)).where(and(eq(properties.slug, slug), eq(properties.publicationStatus, "published")));
