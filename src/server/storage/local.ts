...
function root() { const value = process.env.STORAGE_ROOT; if (!value || !path.isAbsolute(value)) throw new Error("STORAGE_ROOT harus path absolut di luar repository."); const resolved = path.normalize(value); const workspace = process.cwd(); const relative = path.relative(workspace, resolved); if (!relative || (!relative.startsWith(".." + path.sep) && relative !== ".." && !path.isAbsolute(relative))) throw new Error("STORAGE_ROOT tidak boleh di dalam repository."); return resolved; }
